//! @file ac_vec32.h
//!	@brief Vector functions (Word32 type)
...
//! @brief Vector of type Word32 
typedef struct vec32 {
	Word32 *data;	//!< Pointer to the internal data array 
	int datasize;	//!< Vector size
} vec32;

//! @brief Matrix of type float
...
int vec32_set_length(vec32 *v, int size);

//! @brief Get vector length
//! @param v the pointer to the vector
//! @return the size of vector  
int vec32_length(const vec32 *v);
...
int vec32_free(vec32 *v);

//! @brief Clear (set to zeros) vector content 
//! @param v the pointer to the vector
void vec32_clear(vec32 *v);
...
int vec32_copy(const vec32 *src, vec32 *dest); 
...
void vec32_concat(const vec32 *src1, const vec32 *src2, vec32 *dest);
...
//! @remark the size of destination vector shall be automatically adjusted.
void vec32_concat_inplace(const vec32 *src, vec32 *src_dest);
...
int vec32_mid(const vec32 *src, vec32 *dest, int i1, int i2);
...
//! @return FALSE if indexes are out of range, TRUE - otherwise. 
//! @remark use -1 (#VEND) constant to indicate the last element of the vector
int vec32_mid_inplace(vec32 *src_dest, int i1, int i2);
...
Word32 vec32_get_value(const vec32 *v, int i);
...
int mat32_set_size(mat32 *m, int rows, int cols);
...
Word32 mat32_get_value(const mat32 *m, int r, int c);
...
int mat32_get_col(const mat32 *m, vec32 *v, int idx);
...
int mat32_get_row(const mat32 *m, vec32 *v, int idx);
...
//! @param idx the column index
//! @return FALSE if column index is out of range, TRUE - otherwise.
int mat32_set_col(const vec32 *v, mat32 *m, int idx);
...
//! @param idx the row index
//! @return FALSE if row index is out of range, TRUE - otherwise.
int mat32_set_row(const vec32 *v, mat32 *m, int idx);
...
int mat32_copy(const mat32 *src, mat32 *dest); 
